...
processed_count = 0
for split in splits:
    image_dir = os.path.join(vnts_base_dir, split, 'images')
    label_dir = os.path.join(vnts_base_dir, split, 'labels')
    ...
    for img_filename in os.listdir(image_dir):
        ...
        for line in lines:
            ...
            new_class_id = master_map['vnts'].get(class_id, -1)
            if new_class_id == -1:
        ...
        new_img_path = os.path.join(output_dir, 'images', target_split, f"vnts_{img_filename}")
        new_label_path = os.path.join(output_dir, 'labels', target_split, f"vnts_{img_filename.replace('.png', '.txt').replace('.jpg', '.txt')}")
...
print(f"Processed {processed_count} VNTS images.")
